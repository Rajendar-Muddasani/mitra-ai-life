#!/usr/bin/env python3
"""
Generate overview videos for Class 8 Lessons 01-12.
Usage:
    python3 scripts/generate_class08_lessons_01_12.py          # all lessons
    python3 scripts/generate_class08_lessons_01_12.py 01 02    # specific lessons
"""
...
ASSET_DIR = ROOT / "content" / "assets" / "students" / "class-08"
HTML_DIR = ROOT / "content" / "students" / "class-08"
S3_BUCKET = "mitra-ai-life-assets"
S3_PREFIX = "students/class-08"
VIDEO_W, VIDEO_H, FPS = 1280, 720, 24
TTS_LANG = "en-US"
TTS_VOICE = "en-US-Chirp3-HD-Charon"
TTS_RATE = 1.0
BADGE_COL = (13, 148, 136)
BULLET_COL = (94, 234, 212)
OUTLINE_COL = (94, 234, 212, 145)
...
def _make_frame(lesson_data: dict, scene: dict, idx: int, total: int) -> np.ndarray:
    ...
    drw.text((106, 99), lesson_data["badge"].upper(), font=FONT_BADGE, fill=(240, 255, 252, 255))
    drw.text((86, 155), f"{idx:02d}/{total:02d}", font=FONT_FOOTER, fill=(153, 246, 228, 255))
    ...
    drw.text((86, 610), "Mitra AI Life", font=FONT_FOOTER, fill=(153, 246, 228, 255))
...
def _clean_title(text: str) -> str:
    ...
    text = re.sub(r"^Section\s+\d+\s+of\s+\d+\s*", "", text)
...
def _extract_ordered_texts(block: str) -> list[str]:
    token_re = re.compile(
        ...
        r"<p[^>]*>.*?</p>|"
        r"<li[^>]*>.*?</li>|"
        r"<div class=\"story-highlight\"[^>]*>.*?</div>|"
...
def _parse_section(block: str) -> dict | None:
    title = _clean_title(_first_match(r"<h2>(.*?)</h2>", block))
    if not title:
        return None
...
def load_lesson(lesson_id: str) -> dict:
    ...
    subtitle = _strip_html(_first_match(r'<p class="lh-sub">(.*?)</p>', html))
    ...
    story_title = _clean_title(_first_match(r"<h2>(.*?)</h2>", story_block))
    story_texts = _extract_ordered_texts(story_block)
    story_fallback = _fallback_block_text(story_block, story_title)
    if len(" ".join(story_texts)) < 80 and story_fallback:
        story_texts.append(story_fallback)
    story_texts = [text for text in story_texts if text and text != story_title]

    intro_prefix = f"Welcome to Class 8 Lesson {int(lesson_id)} - {title}"
    intro_texts = [subtitle]
    if story_title:
        intro_texts.append(story_title)
    ...
    return {
        "badge": f"CLASS 8 - LESSON {int(lesson_id)}",
        "title": title,
        "image": image,
        "scenes": scenes,
    }
...
def embed_video_in_html(lesson_id: str) -> None:
    ...
    while i < len(html):
        ...
        print(f"  WARNING: Could not find closing </div> in {html_path.name}")
        return
    ...
    if ".video-box {" not in new_html:
        video_box_css = (
            ...
            "overflow: hidden; background: #020617; border: 2px solid rgba(13,148,136,0.35); }\n"
