...
7 scenes, ~120s, voice: en-US-Chirp3-HD-Charon
Output: content/assets/videos/level-07-intro-charon.mp4
"""
...
AUDIO_DIR = ROOT / f"content/assets/videos/audio_tmp_l7{VERSION_SUFFIX}"
OUT       = ROOT / f"content/assets/videos/level-07-intro{VERSION_SUFFIX}.mp4"
...
def build_video():
    ...
    print(f"\nNext — upload to S3:")
    print(f"  source .env && aws s3 cp {OUT} s3://mitra-ai-life-assets/videos/level-07-intro{VERSION_SUFFIX}.mp4 --content-type 'video/mp4' --cache-control 'public, max-age=86400'")
