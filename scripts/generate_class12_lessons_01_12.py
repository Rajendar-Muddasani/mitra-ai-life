#!/usr/bin/env python3
"""
Generate overview videos for Class 12 Lessons 01-12.
Usage:
    python3 scripts/generate_class12_lessons_01_12.py          # all lessons
    python3 scripts/generate_class12_lessons_01_12.py 01 02    # specific lessons
"""
...
ASSET_DIR = ROOT / "content" / "assets" / "students" / "class-12"
HTML_DIR = ROOT / "content" / "students" / "class-12"
S3_BUCKET = "mitra-ai-life-assets"
S3_PREFIX = "students/class-12"
VIDEO_W, VIDEO_H, FPS = 1280, 720, 24
TTS_LANG = "en-US"
TTS_VOICE = "en-US-Chirp3-HD-Charon"
TTS_RATE = 1.0
BADGE_COL = (79, 70, 229)
BULLET_COL = (165, 180, 252)
OUTLINE_COL = (99, 102, 241, 145)
...
def _make_frame(lesson_data: dict, scene: dict, idx: int, total: int) -> np.ndarray:
    ...
    for x in range(0, 690):
        ...
        gd.line([(x, 0), (x, VIDEO_H)], fill=(8, 11, 43, alpha))
    ...
    drw.text((86, 155), f"{idx:02d}/{total:02d}", font=FONT_FOOTER, fill=(199, 210, 254, 255))
    ...
    for bullet in scene["bullets"]:
        ...
        y = _put_lines(drw, lines, 126, y, FONT_BODY, (224, 231, 255, 255), 8)
        y += 14

    drw.text((86, 610), "Mitra AI Life", font=FONT_FOOTER, fill=(199, 210, 254, 255))
...
def _build_intro_scene(
    ...
    intro_prefix = f"Welcome to Class 12 Lesson {int(lesson_id)} - {title}"
...
def load_lesson(lesson_id: str) -> dict:
    ...
    return {
        "badge": f"CLASS 12 - LESSON {int(lesson_id)}",
        "title": title,
        "image": image,
...
def embed_video_in_html(lesson_id: str, lesson_title: str) -> None:
    ...
    section_html = (
        ...
        f'    <h2>Class 12 Lesson {int(lesson_id)} - {lesson_title}</h2>\n'
        f'    {video_box}\n'
    ...
    video_css = (
        ".lesson-video{background:#1e1b4b;padding:2.5rem 1rem;color:#fff;margin:0;}\n"
        ".lesson-video .sec-label{font-size:.72rem;font-weight:700;letter-spacing:.15em;text-transform:uppercase;color:#a5b4fc;margin-bottom:.5rem;text-align:center}\n"
        ...
        ".video-box{position:relative;width:100%;max-width:820px;margin:0 auto;padding-top:56.25%;border-radius:14px;overflow:hidden;background:#020617;border:2px solid rgba(99,102,241,.45)}\n"
