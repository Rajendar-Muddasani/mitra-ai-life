#!/usr/bin/env python3
"""
Generate overview videos for Class 9 Lessons 01-12.
Usage:
    python3 scripts/generate_class09_lessons_01_12.py          # all lessons
    python3 scripts/generate_class09_lessons_01_12.py 01 02    # specific lessons
"""
...
ASSET_DIR = ROOT / "content" / "assets" / "students" / "class-09"
HTML_DIR = ROOT / "content" / "students" / "class-09"
S3_BUCKET = "mitra-ai-life-assets"
S3_PREFIX = "students/class-09"
VIDEO_W, VIDEO_H, FPS = 1280, 720, 24
TTS_LANG = "en-US"
TTS_VOICE = "en-US-Chirp3-HD-Charon"
TTS_RATE = 1.0
BADGE_COL = (249, 115, 22)
BULLET_COL = (253, 186, 116)
OUTLINE_COL = (251, 146, 60, 145)
...
def _make_frame(lesson_data: dict, scene: dict, idx: int, total: int) -> np.ndarray:
    ...
    drw.rounded_rectangle((86, 90, 346, 132), radius=20, fill=(*BADGE_COL, 220))
    drw.text((106, 99), lesson_data["badge"].upper(), font=FONT_BADGE, fill=(255, 247, 237, 255))
    drw.text((86, 155), f"{idx:02d}/{total:02d}", font=FONT_FOOTER, fill=(253, 186, 116, 255))
    ...
    drw.text((86, 610), "Mitra AI Life", font=FONT_FOOTER, fill=(253, 186, 116, 255))
...
def load_lesson(lesson_id: str) -> dict:
    ...
    story_block, _ = _find_div_block(html, '<div class="story-panel">')
    story_label = _clean_title(_first_match(r'<div class="sp-label">(.*?)</div>', story_block))
    story_texts = _extract_ordered_texts(story_block)
    story_fallback = _fallback_block_text(story_block, story_label)
    if len(" ".join(story_texts)) < 80 and story_fallback:
        story_texts.append(story_fallback)
    story_texts = [text for text in story_texts if text and text != story_label]

    intro_prefix = f"Welcome to Class 9 Lesson {int(lesson_id)} - {title}"
    intro_texts = [description]
    if story_label:
        intro_texts.append(story_label)
    intro_texts.extend(story_texts[:3])
    intro_scene = {
        "title": title,
    ...
    parsed_sections = [section for block in section_blocks if (section := _parse_section(block))]
    chosen_sections = [
        parsed_sections[idx]
    ...
    return {
        "badge": f"CLASS 9 - LESSON {int(lesson_id)}",
        "title": title,
        "image": image,
...
def embed_video_in_html(lesson_id: str, lesson_title: str) -> None:
    ...
    section_html = (
        ...
        f'    <h2>Class 9 Lesson {int(lesson_id)} - {lesson_title}</h2>\n'
        f'    {video_box}\n'
    ...
    video_css = (
        ".lesson-video{background:#7c2d12;padding:2.5rem 1rem;color:#fff;margin:0;}\n"
        ".lesson-video .sec-label{font-size:.72rem;font-weight:700;letter-spacing:.15em;text-transform:uppercase;color:#fdba74;margin-bottom:.5rem;text-align:center}\n"
        ".lesson-video h2{color:#fff;font-family:'Baloo 2',cursive;font-size:1.4rem;text-align:center;margin-bottom:1.2rem}\n"
        ".video-caption{color:rgba(255,255,255,.55);font-size:.78rem;text-align:center;margin-top:.7rem}\n"
        ".video-box{position:relative;width:100%;max-width:820px;margin:0 auto;padding-top:56.25%;border-radius:14px;overflow:hidden;background:#020617;border:2px solid rgba(249,115,22,.35)}\n"
