#!/usr/bin/env python3
"""
Generate overview videos for Class 10 Lessons 01-12.
Usage:
    python3 scripts/generate_class10_lessons_01_12.py          # all lessons
    python3 scripts/generate_class10_lessons_01_12.py 01 02    # specific lessons
"""
...
ASSET_DIR = ROOT / "content" / "assets" / "students" / "class-10"
HTML_DIR = ROOT / "content" / "students" / "class-10"
S3_BUCKET = "mitra-ai-life-assets"
S3_PREFIX = "students/class-10"
VIDEO_W, VIDEO_H, FPS = 1280, 720, 24
TTS_LANG = "en-US"
TTS_VOICE = "en-US-Chirp3-HD-Charon"
TTS_RATE = 1.0
BADGE_COL = (5, 150, 105)
BULLET_COL = (110, 231, 183)
OUTLINE_COL = (16, 185, 129, 145)
...
def _make_frame(lesson_data: dict, scene: dict, idx: int, total: int) -> np.ndarray:
    ...
    drw.text((106, 99), lesson_data["badge"].upper(), font=FONT_BADGE, fill=(236, 253, 245, 255))
    drw.text((86, 155), f"{idx:02d}/{total:02d}", font=FONT_FOOTER, fill=(110, 231, 183, 255))
    ...
    drw.text((86, 610), "Mitra AI Life", font=FONT_FOOTER, fill=(110, 231, 183, 255))
...
def load_lesson(lesson_id: str) -> dict:
    ...
    intro_prefix = f"Welcome to Class 10 Lesson {int(lesson_id)} - {title}"
    ...
    return {
        "badge": f"CLASS 10 - LESSON {int(lesson_id)}",
        "title": title,
        "image": image,
...
def embed_video_in_html(lesson_id: str, lesson_title: str) -> None:
    ...
    section_html = (
        ...
        f'    <h2>Class 10 Lesson {int(lesson_id)} - {lesson_title}</h2>\n'
        f'    {video_box}\n'
    ...
    video_css = (
        ".lesson-video{background:#064e3b;padding:2.5rem 1rem;color:#fff;margin:0;}\n"
        ".lesson-video .sec-label{font-size:.72rem;font-weight:700;letter-spacing:.15em;text-transform:uppercase;color:#6ee7b7;margin-bottom:.5rem;text-align:center}\n"
        ...
        ".video-box{position:relative;width:100%;max-width:820px;margin:0 auto;padding-top:56.25%;border-radius:14px;overflow:hidden;background:#020617;border:2px solid rgba(5,150,105,.35)}\n"
