...
Usage:
  ...
  source .env
  python scripts/generate_home_hero_video.py --force
"""
...
AMBER = (245, 158, 11)
GREEN = (16, 185, 129)
MITRA_URL = "https://mitra-ai-life-assets.s3.us-west-2.amazonaws.com/characters/mitra-reference.png"
...
SLIDES = [
    (
        "MITRA AI LIFE",
        "Practical AI and learning skills",
        "One simple home for every Indian family",
        "Mitra AI Life brings practical AI and learning skills into one simple home for every Indian family.",
    ),
    (
        "ONE URL TO SHARE",
        "Daily Life, Students, Tuition, English",
        "Separate tracks, one trusted guide",
        "There are separate tracks for daily AI skills, school students, AI tuition, spoken English, small business, teachers, and college project kits.",
    ),
    (
        "FREE-FIRST LEARNING",
        "Start without pressure",
        "Read, watch, practice, then grow",
        "The first promise is free-first learning. Start with simple lessons, short videos, and practical examples before moving into advanced projects.",
    ),
    (
        "BUILT FOR INDIA",
        "Real examples from daily life",
        "Families, schools, shops, teachers, and students",
        "Every lesson uses real Indian needs: homework, WhatsApp messages, safety, privacy, local business, exams, and career skills.",
    ),
    (
        "SAFE AND HONEST",
        "AI helps. People decide.",
        "Check facts, protect privacy, and use AI responsibly",
        "Mitra AI Life teaches safe and honest AI use. Check facts, protect privacy, avoid cheating, and keep people in control.",
    ),
    (
        "START TODAY",
        "Choose the track that fits you",
        "mitraailife.com",
        "Start today. Open Mitra AI Life, choose the track that fits you, and build one useful skill at a time.",
    ),
]
...
def paste_circle(base: Image.Image, avatar: Image.Image, center: tuple[int, int], size: int) -> None:
    ...
    ring_draw.ellipse((0, 0, size + 23, size + 23), fill=(83, 166, 255, 70), outline=(104, 225, 255, 180), width=5)
    base.alpha_composite(ring, (x - 12, y - 12))
    base.paste(avatar, (x, y), mask)


def render_slide(slide_number: int, eyebrow: str, title: str, subline: str, path: Path) -> None:
    ...
    draw.rounded_rectangle((78, 74, WIDTH - 78, HEIGHT - 74), radius=34, outline=(255, 255, 255, 34), width=2)
    ...
    font_eye = ImageFont.truetype(FONT_BODY, 34)
    font_title = ImageFont.truetype(FONT_HEADING, 88)
    font_sub = ImageFont.truetype(FONT_BODY, 46)
    font_brand = ImageFont.truetype(FONT_BODY, 30)
    font_step = ImageFont.truetype(FONT_BODY, 28)

    eye_width = draw.textbbox((0, 0), eyebrow, font=font_eye)[2]
    eye_y = 290
    draw.rounded_rectangle(((WIDTH - eye_width) / 2 - 28, eye_y - 12, (WIDTH + eye_width) / 2 + 28, eye_y + 52), radius=20, outline=BLUE + (180,), width=3)
    ...
    for line in title_lines:
        ...
        draw.text(((WIDTH - line_width) / 2 + 4, title_y + 4), line, fill=(0, 0, 0, 190), font=font_title)
        draw.text(((WIDTH - line_width) / 2, title_y), line, fill=INK + (255,), font=font_title)
    ...
    for line in sub_lines:
        ...
        line_width = bbox[2] - bbox[0]
        draw.text(((WIDTH - line_width) / 2, sub_y), line, fill=MUTED + (255,), font=font_sub)
    ...
    draw.text((WIDTH - brand_width - 88, HEIGHT - 76), brand, fill=MUTED + (210,), font=font_brand)
